...
int mem_init_from_dumpfile(const char* filename, void** memory, size_t* mem_capacity_in_bytes){
    ...
    // correcteur free file when error
...
static int page_file_read(
	...
	fseek(page_file, 0L, SEEK_END); // correcteur: see PAGE_SIZE
  ...
  // correcteur what if phy addr 32b is not alligned to the beginning of a page?
