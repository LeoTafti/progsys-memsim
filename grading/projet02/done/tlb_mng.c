...
int tlb_flush(tlb_entry_t * tlb) {
  ...
  for(size_t i = 0; i < TLB_LINES; i++) { // correcteur could use memset
    tlb[i].v = INVALID;
    tlb[i].tag = 0;
...
int tlb_hit(const virt_addr_t * vaddr,
  ...
  uint64_t tag = virt_addr_t_to_uint64_t(vaddr)>>PAGE_OFFSET; // correcteur to_virtual_page_number
  node_t* m = NULL;
...
int tlb_insert( uint32_t line_index,
  ...
  M_REQUIRE_NON_NULL(tlb);
  // correcteur check line_index, otherwise can SEGFAULT
...
int tlb_entry_init( const virt_addr_t * vaddr,
  ...
  tlb_entry->v = VALID;
  tlb_entry->tag = virt_addr_t_to_uint64_t(vaddr)>>PAGE_OFFSET;// correcteur use virt_addr_t_to_vortual_page_number
...
int tlb_search( const void * mem_space,
  ...
  *hit_or_miss = tlb_hit(vaddr, paddr, tlb, replacement_policy);
  ...
  if(*hit_or_miss == MISS) {
      ...
      tlb_entry_init(vaddr, paddr, &new_entry); // correcteur check errors here

      //insert in tlb at the front's line index
      node_t* lru = replacement_policy->ll->front;
      tlb[lru->value] = new_entry;// correcteur should be tlb_insert
